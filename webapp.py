...
def home():
    # 你可以把這段維持你自己的 UI 版本
    # 我這裡放一個乾淨版（如果你已經有 Lv1 升級包 UI，就保留你自己的也行）
    return """
<!doctype html>
<html lang="zh-Hant">
<head>
...
async function run(){
  ...
  try{
    ...
    document.getElementById("lv2").textContent = JSON.stringify(lv2, null, 2);
...
def _normalize_result(d: Dict[str, Any]) -> Dict[str, Any]:
    """
    讓 analyze_text 回來的資料「就算少欄位」也不會害 response_model 爆炸。
    """
...
async def analyze(body: AnalyzeRequest, req: Request):
    ...
    try:
        out = analyze_text(text, context=body.context)
        return _normalize_result(out)
    except Exception as e:
        # 不要把 traceback 噴出去（Render log 你看得到就好）
